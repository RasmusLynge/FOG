...
public class LogicFacade {
    ...
    /**
     * This method is used to create an arraylist of orders for the salesrep to
     * view. it takes a state as parameter because thats how we sort orders.
     * there are 5 different states: requested, awaiting salesrep, paid, shipped
     * and cancelled orders.
     *
     * @param state this string holds the given state needed to find the
     * specific orders.
     * @return an arraylist with statespecific orders.
     * @throws DMException if function catches ClassNotFoundException |
     * SQLException
     */
    public ArrayList<Order> getSpecificOrders(String state) throws DMException {
        return df.getSpecificOrders(state);
    }

    /**
     * This method is used to login the salesrep so he can handle orders on the
     * website it takes an email and a password to login so we have those as our
     * parameters
     *
     * @param email this string is the input email
     * @param password this string is the input password
     * @return when it has finished logging in the method then returns the user
     * object so we can set it on a session.
    ...
    public ArrayList<Order> getAllOrders() throws DMException {
        return df.getAllOrders();
    }

    /**
     * This method takes a whole bunch of parameters saves them to the created
     * Order object and calculates the price then it saves the order in the
     * database
     *
     ...
     * @param highRoof this boolean decides if the carport needs a top roof or
     * not
     * @param shedLength this int holds the customers desired shed length
     * @return a finished order object
     * @throws DMException if the dm function catches ClassNotFoundException |
     * SQLException
    ...
    /**
     * This method is called when the salesrep wants to change some details to a
     * specific order it then changes whatever needs changing, updates the price
     * and the rest of the details and return the refreshed order object
     *
     * @param orderId the orderid needed to find the specific order in the DB
     * @param desiredWidth this is the input desired width
     * @param desiredLength this is the input desired length
     * @param flatRoof this int is a boolean: 1 means the carport needs a
     * toproof. 0 is flatroof
